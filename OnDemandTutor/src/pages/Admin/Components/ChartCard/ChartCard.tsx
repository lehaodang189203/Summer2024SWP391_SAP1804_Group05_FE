...
export default function ChartCard({dataChart}:ChartCardProps) {
  ...
  const generateDataForPeriod = (date: Date, period: 'month' | 'week'): ChartData[] => {
    ...
    if (period === 'month') {
      for (let i = 0; i < 12; i++) {
        ...
        // Tính tổng doanh thu của các tuần trong tháng
        const monthData = dataChart.filter(data => data.month === currentDate.getMonth() + 1 && data.year === currentDate.getFullYear());
        const totalRevenue = monthData.reduce((acc, data) => acc + data.revenue, 0);

        newData.push({
          name: monthName,
          date: `${format(monthStart, 'dd/MM/yyyy')} - ${format(monthEnd, 'dd/MM/yyyy')}`,
      ...
      for (let i = 0; i < 4; i++) {
        ...
        const weekData = dataChart.find(data => 
          data.date === `${format(weekStart, 'dd/MM/yyyy')} - ${format(weekEnd, 'dd/MM/yyyy')}` 
   ...
   // Hàm format số liệu trên trục Y
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={400}>
        <BarChart data={data}>
          ...
          <Tooltip formatter={(value: number) => `${value} VNĐ`} />
